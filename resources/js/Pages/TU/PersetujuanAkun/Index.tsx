...
interface Permohonan {
    ...
    password_sementara?: string | null;
    status_email?: 'belum_dikirim' | 'terkirim' | 'gagal';
...
export default function Index({ permohonans, counts, filters }: Props) {
    ...
    const [resendEmailModal, setResendEmailModal] = useState<Permohonan | null>(null);
    const [deleteConfirmModal, setDeleteConfirmModal] = useState<Permohonan | null>(null);
    ...
    const handleDelete = () => {
        ...
        setIsSubmitting(true);
        router.delete(route('tu.persetujuan-akun.destroy', deleteConfirmModal.id), {
            onFinish: () => {
                setIsSubmitting(false);
    ...
    const handleResendEmail = () => {
        if (!resendEmailModal) return;
        setIsSubmitting(true);
        router.post(route('tu.persetujuan-akun.resend-email', resendEmailModal.id), {}, {
            onFinish: () => {
                setIsSubmitting(false);
                setResendEmailModal(null);
            }
        });
    };
    ...
    return (
        <TULayout>
            ...
            <div className="space-y-6">
                ...
                {/* Flash Success / Warning / Error Alerts */}
                {flash?.success && (
                ...
                {flash?.warning && (
                    <div className="p-4 rounded-2xl bg-amber-50 border border-amber-300 text-amber-950 text-xs font-medium flex items-start gap-3 shadow-sm animate-in fade-in">
                        <AlertTriangle className="w-5 h-5 flex-shrink-0 text-amber-600 mt-0.5" />
                        <div className="leading-relaxed font-medium">{flash.warning}</div>
                    </div>
                )}
                {flash?.error && (
                                                ...
                                                {item.status === 'disetujui' && (
                                                    <div className="flex flex-col items-center gap-1">
                                                        ...
                                                        {item.status_email === 'terkirim' && (
                                                            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600" title="Email kredensial berhasil terkirim ke mahasiswa">
                                                                <Mail className="w-3 h-3" /> Email Terkirim
                                                            </span>
                                                        )}
                                                        {item.status_email === 'gagal' && (
                                                            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full border border-red-200" title={item.error_email || 'Email gagal terkirim ke server'}>
                                                                <AlertCircle className="w-3 h-3" /> Email Gagal
                                                            </span>
                                                        )}
                                                    </div>
                                                )}
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1.5">
                                                    ...
                                                    <button
                                                        ...
                                                        title="Lihat Detail & Password Sementara"
                                                    >
                                                        <Eye className="w-4 h-4" />
                                                    </button>

                                                    {/* Kirim Ulang Email Button for Approved Items */}
                                                    {item.status === 'disetujui' && (
                                                        <button
                                                            type="button"
                                                            onClick={() => setResendEmailModal(item)}
                                                            className="p-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition-colors cursor-pointer"
                                                            title="Kirim Ulang Email Kredensial Baru"
                                                        >
                                                    ...
                                                    <button
                                                        ...
                                                        className="p-1.5 rounded-lg bg-slate-100 hover:bg-red-50 hover:text-red-600 text-slate-400 transition-colors cursor-pointer"
                        ...
                        <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
                            ...
                            <div className="space-y-3 text-xs">
                                <div className="p-3 bg-slate-50 rounded-xl space-y-2">
                                    ...
                                    {/* Temporary Password & Email Status Box for Approved Accounts */}
                                        ...
                                        <div className="pt-2 border-t border-slate-200 space-y-2">
                                            <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-blue-700 text-[10px] uppercase font-bold flex items-center gap-1">
                                                        <Key className="w-3.5 h-3.5 text-blue-600" /> Password Sementara Mahasiswa
                                                    </span>
                                                ...
                                                {detailModal.password_sementara ? (
                                                    <div className="flex items-center justify-between gap-2 bg-white px-3 py-2 rounded-lg border border-blue-200">
                                                        <span className="font-mono font-black text-slate-900 text-sm tracking-widest select-all">
                                                            {detailModal.password_sementara}
                                                        </span>
                                                        <button
                                                            type="button"
                                                            onClick={() => {
                                                                navigator.clipboard.writeText(detailModal.password_sementara || '');
                                                                alert(`Password sementara (${detailModal.password_sementara}) berhasil disalin!`);
                                                            }}
                                                            className="px-2.5 py-1 text-[11px] bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold rounded-md flex items-center gap-1 cursor-pointer transition-colors shadow-xs"
                                                    ...
                                                    <div className="bg-white/80 px-3 py-2 rounded-lg border border-blue-200 text-slate-500 italic text-[11px]">
                                                        Mahasiswa telah berhasil mengganti password pribadinya saat login.
                                                    </div>
                                                )}

                                                <div className="flex items-center justify-between text-[11px] text-slate-600 pt-0.5">
                                                    <span>Status Pengiriman Email:</span>
                                                    {detailModal.status_email === 'terkirim' ? (
                                                        <span className="font-bold text-emerald-700 flex items-center gap-1">
                                                            <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Berhasil Terkirim
                                                        </span>
                                                    ) : detailModal.status_email === 'gagal' ? (
                                                        <span className="font-bold text-red-600 flex items-center gap-1" title={detailModal.error_email || ''}>
                                                            <AlertCircle className="w-3 h-3 text-red-600" /> Gagal ({detailModal.error_email || 'Kendala jaringan'})
                                                        </span>
                                                    ) : (
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setDetailModal(null);
                                                    setResendEmailModal(target);
                                                }}
                                                className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-sm"
                                            >
                                                <Send className="w-3.5 h-3.5" />
                                                <span>Kirim Ulang Email Kredensial Baru</span>
                                            </button>
                                        </div>
                                    )}
                                </div>
                ...
                {/* MODAL: RESEND EMAIL CONFIRMATION */}
                {resendEmailModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
                        <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
                            ...
                            <div className="text-center space-y-1">
                                <h3 className="text-lg font-black text-slate-900">Kirim Ulang Email Kredensial?</h3>
                                <p className="text-xs text-slate-500 leading-relaxed">
                                    Sistem akan membuat password sementara baru untuk mahasiswa <strong className="text-slate-800">{resendEmailModal.nama}</strong> ({resendEmailModal.nim}), menyimpannya di TU, dan mengirimkannya kembali ke email <strong className="text-slate-800">{resendEmailModal.email}</strong>.
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                <button
                                    type="button"
                                    onClick={() => setResendEmailModal(null)}
                                    className="flex-1 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-xs cursor-pointer"
                                >
                                    Batal
                                </button>
                                <button
                                    ...
                                    className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-md disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer"
                                >
                                    {isSubmitting ? (
                                        <>
                ...
                {/* MODAL: KONFIRMASI HAPUS PERMOHONAN */}
                {deleteConfirmModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
                        <div className="bg-white rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95 text-center">
                            ...
                            <div className="space-y-1">
                                <h3 className="text-base font-bold text-slate-900">Hapus Permohonan Akun?</h3>
                                <p className="text-xs text-slate-500 leading-relaxed">
                                    Apakah Anda yakin ingin menghapus data permohonan akun dari <strong className="text-slate-800">{deleteConfirmModal.nama}</strong> ({deleteConfirmModal.nim})?
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                ...
                                <button
                                    ...
                                    disabled={isSubmitting}
                                    className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs shadow-md shadow-red-600/20 disabled:opacity-50 flex items-center justify-center gap-1.5"
                                >
                                    {isSubmitting ? (
                                        <>
