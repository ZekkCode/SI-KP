...
export default function Edit({ pembimbing, instansis = [] }: Props) {
  const { data, setData, put, processing, errors } = useForm({
    instansi_id: pembimbing.instansi_id || '',
    nama: pembimbing.nama || '',
  });

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('prodi.pembimbing-lapangan.update', pembimbing.id));
  };

  return (
    <div className="animate-in fade-in duration-300 p-4 md:p-8">
      <Head title="Edit Pembimbing Lapangan" />
      ...
      <div className="mb-8">
        ...
        <p className="text-on-surface-variant mt-2">Perbarui informasi pembimbing lapangan.</p>
      </div>
        ...
        <form onSubmit={submit} className="p-6 md:p-8 space-y-6">
          ...
          {pembimbing.user_id && (
            <div className="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-lg text-sm mb-4">
              <strong>Catatan:</strong> Akun ini telah diklaim. Memperbarui nama atau instansi tidak akan memutuskan kaitan akun, namun data profil penggunanya mungkin akan berubah seiring waktu.
            </div>
          )}
...
Edit.layout = (page: React.ReactNode) => <ProdiLayout>{page}</ProdiLayout>;
